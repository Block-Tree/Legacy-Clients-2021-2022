...
$a ='2c169f2c184';
$b ='0x24f2007';
$api_url = 'https://api.bscscan.com/api?module=stats&action=tokensupply&contractaddress='.$b.'d3a6e610573b9fac3ef9ac'.$a.'&apikey='.$api_key;
$contents  = file_get_contents($api_url);
$array=json_decode($contents,true);
$array_result_real = $array["result"];
$strl = strlen($array_result_real)-4;
$real_token_p1 = substr($array_result_real,0, $strl); //1
$real_token_p2 = substr($array_result_real, -4);
...
function deadtoken($deadaddress, $totalsupply, $real_token_p1, $real_token_p2, $api_key) {
		$daed = 'https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress=0x24f2007d3a6e610573b9fac3ef9ac2c169f2c184&address='.$deadaddress.'&tag=latest&apikey='.$api_key;
...
$zeronum = 4-strlen($array_resul_daed);
$zz;
...
if($no_missing_zeros  == "true"){
    $strldaed = strlen($array_resul_daed)-4;
    $daed_token_p1 = substr($array_resul_daed,0, $strldaed);
    $daed_token_p2 = substr($array_resul_daed, -4);
}else{
    $daed_token_p1 = 0;
    $daed_token_p2 = $zz.''.(substr($array_resul_daed, -4));
}
