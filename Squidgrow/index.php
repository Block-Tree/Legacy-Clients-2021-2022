...
$a ='5ed0153C765';
$b ='0x8847918';
$api_url = 'https://api.bscscan.com/api?module=stats&action=tokensupply&contractaddress='.$b.'6BAC914E4313389a64881F'.$a.'&apikey='.$api_key;
$contents  = file_get_contents($api_url);
$array=json_decode($contents,true);
$array_result_real = $array["result"];
$strl = strlen($array_result_real)-19;
$real_token_p1 = substr($array_result_real,0, $strl); //1
$real_token_p2 = substr($array_result_real, -19);
...
function deadtoken($deadaddress, $totalsupply, $real_token_p1, $real_token_p2, $api_key) {
		$daed = 'https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress=0x88479186BAC914E4313389a64881F5ed0153C765&address='.$deadaddress.'&tag=latest&apikey='.$api_key;
...
$zeronum = 19-strlen($array_resul_daed);
$zz;
...
if($no_missing_zeros  == "true"){
    $strldaed = strlen($array_resul_daed)-19;
    $daed_token_p1 = substr($array_resul_daed,0, $strldaed);
    $daed_token_p2 = substr($array_resul_daed, -19);
}else{
    $daed_token_p1 = 0;
    $daed_token_p2 = $zz.''.(substr($array_resul_daed, -19));
}
